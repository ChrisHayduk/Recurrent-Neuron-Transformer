...
class VanillaTransformerModel(nn.Module):
    def __init__(self, vocab_size, max_seq_length, d_model=512, nhead=8, num_decoder_layers=6, dim_feedforward=2048):
        ...
        self.transformer_decoder = nn.TransformerEncoder(transformer_encoder, num_layers=num_decoder_layers, norm=nn.LayerNorm([max_seq_length, d_model]))
    ...
    def forward(self, tgt, tgt_mask=None):
        if tgt_mask is None:
            tgt_seq_len = tgt.size(1)  # Assuming batch_first=True, change size index to 1
            tgt_mask = generate_square_subsequent_mask(tgt_seq_len).to(tgt.device)
        
        tgt = self.embedding(tgt) * math.sqrt(self.d_model)
        tgt = self.pos_encoder(tgt)
        output = self.transformer_decoder(tgt, mask=tgt_mask)
