...
class RecurrentNeuronDataset(Dataset):
    def __init__(self, tokens, seq_length, bpe_tokenizer, vocab_size):
        self.tokens = tokens
    ...
    def __getitem__(self, idx):
        ...
        target_seq = torch.tensor(self.tokens[idx+1 : idx+1+self.seq_length])
        return input_seq, target_seq


class RecurrentNeuronDataLoader:
    def __init__(self, file_path, seq_length, bpe_tokenizer, batch_size, vocab_size, split_ratio=0.8):
    ...
    def _create_datasets(self):
        ...
        train_dataset = RecurrentNeuronDataset(train_tokens, self.seq_length, self.bpe_tokenizer, self.vocab_size)
        test_dataset = RecurrentNeuronDataset(test_tokens, self.seq_length, self.bpe_tokenizer, self.vocab_size)
...
if __name__ == '__main__':
    ...
    bpe_tokenizer = 'gpt2'
    vocab_size = 50257
    data_loader = RecurrentNeuronDataLoader(file_path, seq_length, bpe_tokenizer, batch_size, vocab_size)
